...
export default {
  ...
  methods: {
    ...
    deleteLink(id) {
      ...
      FriendLinkAPI.deleteFriendLink(param).then(res => {
        if (res.flag) {
    ...
    addOrEditCategory() {
      ...
      FriendLinkAPI.saveOrUpdateFriendLink(this.linkForm).then(res => {
        if (res.flag) {
          this.$notify.success(res.message)
          this.listLinks()
        } else {
    ...
    listLinks() {
      FriendLinkAPI.getListFriendLink(this.keywords, this.current, this.size).then(res => {
