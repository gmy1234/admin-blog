...
export default {
  ...
  methods: {
    ...
    updateCommentReview(id) {
      ...
      param.isReview = 1
      CommentAPI.reviewComments(param).then(({ data }) => {
        if (data.flag) {
    ...
    deleteComments(id) {
      ...
      CommentAPI.deleteComment(param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: '成功',
    ...
    // 评论列表
    listComments() {
      CommentAPI.listComments(this.current, this.size, this.keywords, this.type, this.isReview)
        .then(({ data }) => {
          console.log(data)
